...
#function to generate stats for regression models
def gen_reg_rpt(modelName, my_y_test, my_y_pred):
    myMse = mean_squared_error(my_y_test_cm, my_y_pred)
    myR2 = r2_score(my_y_test, my_y_pred)
    myExplainedVar = explained_variance_score(my_y_test, my_y_pred)
    myMape = mean_absolute_percentage_error(my_y_test, my_y_pred)
    myMae = median_absolute_error(my_y_test, my_y_pred)
    print("------------------------------------------------")
    print('Regression Model Performance Report: ' + modelName)
    print(f"Mean Squared Error:             {round(myMse,4)}")
    print(f"R2 Score:                       {round(myR2,4)}")
    print(f"Explained Variance:             {round(myExplainedVar,4)}")
    print(f"Mean Absolute Percentage Error: {round(myMape,4)}")
    print(f"Median Abolute Error:           {round(myMae,4)}")
    print("------------------------------------------------")
    
## function to generate Confusion Matrix and Classification Report
def gen_cm_cr(modelName, my_y_test, my_y_pred):
    cm = confusion_matrix(my_y_test, my_y_pred)
    cm_df = pd.DataFrame(
        cm, index=["Loyal 0", "Churn 1"], columns=["Predicted Loyal 0", "Predicted Churn 1"]
    )
    
    # Calculating the accuracy score
    acc_score = accuracy_score(my_y_test, my_y_pred)
    bal_acc_score = balanced_accuracy_score(my_y_test, my_y_pred)
